...
update Funcionarios set senha = 'Gere7098' where cargo = 'Gerente de Vendas'
update Funcionarios set senha = 'Coor1314' where cargo = 'Coordenador de Vendas Internas'
...
as if (SELECT count(*) from Funcionarios where Sobrenome = @sobrenome AND senha = @senha) != 0
begin 
	print @sobrenome
	select * from Funcionarios where Sobrenome = @sobrenome AND senha = @senha
	end
	else
	select 'Login Inválido' as msg

exec usp_login 'Buchanan', 'Gere7098'

--PROCEDURE INSERT COORDENADOR
go
create proc usp_InsertCoo
@NumPed    INT,
@CodCli    CHAR(5),
@CodFun    INT,
@DataPed DATE,
...
as
if(@NumPed not in(select NumPed from Pedidos)) and @CodCli in(select CodCli from Clientes) and @CodFun in(select CodFun from Funcionarios) and
@DataPed<@DataEntrega and @Frete >0.0
begin
insert Pedidos values (@NumPed,@CodCli,@CodFun,@DataPed,@DataEntrega,@Frete);
...
exec usp_InsertCoo 555555, 'ALFKI', 1, '10/10/2022', '12/10/2022', 20.00

--PROCEDURE INSERT GERENTE
go
create proc usp_InsertGere
@Descr    VARCHAR(40),
@CodFor    INT ,
@CodCategoria INT ,
@Preco    MONEY, --DEFAULT 0.0,
...
as
if @CodFor in(select CodFor from Fornecedores) and @CodCategoria in(select CodCategoria from Categorias) and
@Preco>0.0 and @Unidades >0 and @Descontinuado = 1 or @Descontinuado=0 and len(@Descr)>0 and @Descr not in(select Descr from Produtos)
begin
insert Produtos values (@Descr,@CodFor,@CodCategoria,@Preco,@Unidades,@Descontinuado);
select 'sucesso' as status
end
else
begin
...
exec usp_InsertGere 'yuyuyuyuay', 1, 1, 2.00, 5, 0

--PROCEDURE CONSULTA COORDENADOR
go
create proc usp_PesquisaCoo
@data date
as
Select DATEDIFF(DAY,DataPed,DataEntrega) as 'Tempo levado para Entrega',DATENAME(WEEKDAY,DataEntrega) as 'Dia da Semana de Entrega', DataPed, DataEntrega from Pedidos
where MONTH(DataEntrega) = MONTH(@data) and YEAR(DataEntrega) = YEAR(@data) 

exec usp_PesquisaCoo '1996/07/15'

--PROCEDURE CONSULTA GERENTE
go
create proc usp_PesquisaGere
@descri varchar(40)

as
select empresa from Fornecedores as f
    inner join Produtos as p on f.CodFor = p.CodFor
        where Descr like '%'+@descri+'%'

exec usp_PesquisaGere 'a'