...
--2)Faça uma lista que mostre todos os imóveis que custam mais que a média de preço dos imóveis. OK
select * from Imovel 
	where VlPreco > (select avg(VlPreco) from Imovel)

--3) Faça uma lista com todos os compradores que tenham ofertas cadastradas com valor superior a 70 mil. OK
select NmComprador from Comprador 
	where CdComprador in 
		(select CdComprador from Oferta
			where VlOferta > 70000)

--4) Faça uma lista com todos os imóveis com oferta superior à média do valor das ofertas. OK
...
--7) Faça uma lista com os imóveis que têm o preço igual ao menor de todos os vendedores, exceto os imóveis do próprio vendedor.
select CdImovel, NmEndereco, VlPreco from Imovel i
	where VlPreco = (select min(VlPreco) from Imovel
		where CdVendedor != i.CdVendedor)

--8) Faça uma lista com as ofertas menores que todas as ofertas do comprador 2, exceto as ofertas do próprio comprador.
select CdImovel, CdComprador, VlOferta from Oferta
...
--9) Faça uma lista de todos os imóveis, onde o Estado e cidade sejam os mesmos do vendedor 3, exceto os imóveis do vendedor 3.
...
--10) Faça uma lista com todos os nomes de bairro onde os imóveis sejam do mesmo estado, cidade e bairro do imóvel 5