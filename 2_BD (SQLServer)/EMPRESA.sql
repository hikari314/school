...
IF EXISTS (SELECT name FROM master.dbo.sysdatabases WHERE name = 'Empresa2')
...
select Nome, month(DataNasc) from Funcionarios 

select Nome, year(DataNasc) from Funcionarios

select dateadd(day, 5, DataNasc) from Funcionarios

select dateadd(month, 5, DataNasc) from Funcionarios

select dateadd(year, 5, DataNasc) from funcionarios

select datediff(day, DataNasc, getdate()) from Funcionarios

select datediff(month, DataNasc, getdate()) from Funcionarios

select datediff(year, DataNasc, getdate()) from Funcionarios

select datename(weekday, DataNasc) from Funcionarios

--LISTA EXERCICIOS DATE----------------------------------------------------------------------------------------------------------------------------------------
--1 – Exiba o nome e data de nascimento dos funcionários, separando dia, mês e ano.
select Nome, day(DataNasc) as 'Day',
 month(DataNasc) as 'Month',
 year(DataNasc) as 'Year' from Funcionarios

--2 – Exiba o nome dos meses da data de nascimento dos funcionários ordenados.
select datename(month, DataNasc) as 'NameMonth' from Funcionarios order by month(DataNasc)

--3 – Exiba o código e nome de todos os funcionários que nasceram em 1987.
select CodFun,Nome from Funcionarios
	where year(DataNasc) = 1987

--4 – Exiba o nome e dias de nascimento dos funcionários que nasceram em abril de 1987.
select Nome, day(DataNasc) from Funcionarios 
	where month(DataNasc) like 'abril' and year(DataNasc) = 1987        --reconhece tanto 4 quanto 'abril', mas como é string, é melhor usar abril

--5 – Exiba o nome e a data de nascimento dos funcionários acrescida de 2 meses.
select Nome, dateadd(month, 2, DataNasc) as 'See2Months' from Funcionarios   --não atualiza a idade, só mostra durante  select, só mudaria a idade se usasse o update

--6 – Exiba o nome e a idade dos funcionários, calculando a idade em relação à data de nascimento e a data de hoje.
select Nome, datediff(year, DataNasc, getdate()) as 'Age' from Funcionarios  --se fosse dias, usava day ao inves de year
...
--LISTA JOIN---------------------------------------------------------------------------------------------------------------------------------------------------
--1 - Mostre todos os dados dos clientes que fizeram pedidos em 1996.  | nao coloca distinct porque o ciente pode ter feito varios pedidos no mesmo ano  - prova 
select c.* from Clientes C
	inner join Pedidos P
		on C.CodCli = P.CodCli 
			and year(P.DataPed) = 1996

--2 – Apresente todos os nomes dos funcionários que fizeram pedidos para o cliente “Around the horn”.  | a tabela Pedidos faz a ponte entre as tabelas  - prova 
select F.Nome from Funcionarios as F
	inner join Pedidos P on F.CodFun = P.CodFun
...
select p.* from Pedidos P
	inner join Clientes C 
		on P.CodCli = C.CodCli
			and C.Nome like 'Comércio Mineiro'

--4 – Exiba todos os dados dos funcionários que participaram dos pedidos de setembro de 1996.
select f.* from Funcionarios F
	inner join Pedidos P
		on F.CodFun = P.CodFun
			and month(P.DataPed) = 9 and year(P.DataPed) = 1996

--5 – Apresente todos os dados e as categorias de todos os produtos da categoria “laticínios”.
select p.*, c.Descr from Produtos P
	inner join Categorias C 
		on P.CodCategoria = C.CodCategoria 
			and C.Descr = 'laticínios'

--6 – Exiba todos os dados dos produtos e o número do pedido de que eles fizeram parte da data “1996-07-08”.  | prova
select p.*, Pd.NumPed as NumeroPedido from Produtos P
...
select F.Nome, P.NumPed as NumeroPedido from Funcionarios F
	inner join Pedidos P 
		on F.CodFun = P.CodFun 
...
select F.Nome, P.* from Funcionarios F 
	inner join Pedidos P 
		on F.CodFun = P.CodFun 
			and F.Salario > 10000

--9 – Apresente o número dos pedidos feito em maio de 1997 e o nome de todos os clientes desses pedidos.  | estudar funcao de data
select P.NumPed, C.Nome from Pedidos P
	inner join Clientes C on P.CodCli = C.CodCli 
		and month(P.DataPed) = 5 and year(P.DataPed) = 1997

--10 – Mostre a descrição da categoria e do produto (uma única vez) dos pedidos com quantidade menor ou igual a 10, feitos em 1998, em ordem decrescente das descrições.   |  prova
select distinct C.Descr, P.Descr from Categorias C
	inner join Produtos P
		on C.CodCategoria = P.CodCategoria
			inner join DetalhesPed DP
				on P.CodProd = DP.CodProd and DP.Qtde <= 10
					inner join Pedidos Pd 
						on DP.NumPed = Pd.NumPed and year(Pd.DataPed) = 1998
							order by C.Descr desc 

--11 – Mostre todos os detalhes dos pedidos entregues em 1997.
select DP.* from DetalhesPed DP 
	inner join Pedidos P on DP.NumPed = P.NumPed	
...
select distinct C.Descr as DescricaoCategoria, P.Descr as DescricaoProduto
...
--e) Senão, desconto = 9.
Declare @qtde int = (select Qtde from DetalhesPed)

if @qtde < 10
	begin 
		update DetalhesPed 
		set Desconto = Desconto - 0.00
		Print @qtde 
		Print Desconto
	end
	else if @qtde < 30
	...
	begin
		update DetalhesPed
		set Desconto = Desconto - 9.00
		Print @qtde 
		Print Desconto
--4 – Para cada cliente mostre nome, país e informe “importação” para países diferentes do Brasil e “exportação” para clientes do Brasil. (usar o banco empresa)
Declare @imex Tinyint 
Select @imex = Pais from Clientes

Declare @nome Tinyint 
Select @nome = Nome from Clientes

Declare @pais Tinyint 
Select @pais = Pais from Clientes
...
--5 – Atribua seu nome a uma variável e exiba seu nome e a quantidade de caracteres.
...
Exec usp_BuscaCliente "ana"

--drop procedure ups_BuscaCliente

--ex2
go
create proc usp_ContaCliente
@nomeBusca2 varchar(50)
as
declare @contagem int, @mensagem char(100)
select @contagem = count(*) from Clientes
	where nome like '%' + @nomeBusca2 + '%'
	if @contagem = 0
		begin
			select @mensagem = 'Nenhum cliente contém "' + @nomeBusca2 + '"'
		end
	else
...
--"corrigir a de cima -> com parametros já | é melhor colcoar create do que alter pq reeescreve tudo"
go
create procedure usp_insertDetalhesPed
	@numPed int,
	@codProd int,
	@preco decimal(17,2),
	@qtde int,
	@desconto dec(17,2)
as
	if (@numPed <> 0) or (@codProd <> 0) --<> é pra ser diferente
	begin
...
exec usp_insertDetalhesPed 11074, 1, 100.25, 25, 10
exec usp_insertDetalhesPed 0, 0, 0, 0, 0

--ex4 | procedure pra login e senha | logger aqui seria um apelido
go
...
as
	...
	SET @getLogin = (select usuario_login from tblUsuario
									where usuario_login = @login)
	SET @getSenha = (select usuario_senha from tblUsuario
									where usuario_login = @login
										and usuario_senha = @senha)

	--condicional pra retornar o login
...
--EXERCICIOS (prova -> exercicio 2 ou 3)
--a - receba o código do funcionário e retorne seu nome, sobrenome e cargo.
go
create procedure usp_NSC2
	@codFunci int

	as
	declare @getnome varchar(10), @getsobrenome varchar(20), @getcargo varchar(30)

	set @getnome = (select Nome from Funcionarios)
	set @getsobrenome = (select Sobrenome from Funcionarios)
	set @getcargo = (select Cargo from Funcionarios)

	begin
		print '@getnome, @getsobrenome, @getcargo'
	end

	exec usp_NSC1 '5'

/*declare @recebeCod int
set @recebeCod = (select CodFun from Funcionarios)
begin
	print */
	
--b – inserir um registro na tabela fornecedores.
go
create procedure usp_insertFornecedor2
	@codFor int,
	@empresa varchar(40),
	@contato varchar(30),
	@cargo varchar(30),
...
exec usp_insertFornecedor2 30, 'Lalaland', 'Stacy', 'Gerente', 'Rua A', 'Cidade B', '012345', 'Brazil'
exec usp_insertFornecedor2 0, 0, 0, 0, 0, 0, 0 ,0
...
go
create procedure usp_insertDetalhesPed2
	@numPed int,
	@codProd int,
	@preco decimal(17,2),
	@qtde int,
...
--d – aumentar o preço de determinado produto, de acordo com o percentual fornecido. Solicitar o código e o percentual de aumento, 
--se nenhum código for informado, aumentar todos os produtos.
go
create proc usp_Aumento
@percentual float,
@codProd int

as
	if (@codProd in(select CodProd from Produtos))
	begin
		update Produtos set Preco = preco * @percentual + preco
